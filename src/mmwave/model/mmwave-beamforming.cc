...
MmWaveBeamforming::SetChannelMatrix (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
	...
	// Carlos modification
	//Add channel condition to the beam sweeping map.
	//If it exist, do not erase the bf vectors, only update channel matrix
	iter = m_channelScanningMatrixMap.find(key);
	if (iter != m_channelScanningMatrixMap.end ())
	{
		iter->second->m_channelMatrix.m_enbSpatialMatrix = g_enbSpatialInstance.at (randomInstance);
		iter->second->m_channelMatrix.m_ueSpatialMatrix = g_ueSpatialInstance.at (randomInstance);
		iter->second->m_channelMatrix.m_powerFraction = g_smallScaleFadingInstance.at (randomInstance);
		iter->second->m_beam = GetLongTermFading (iter->second);
	}
	else
	{
		m_channelScanningMatrixMap.insert(std::make_pair(key,bfParams));
	}
	// End of modification
}
...
//void
//MmWaveBeamforming::LoadCodebookFiles (NetDeviceContainer enbDevices, NetDeviceContainer ueDevices)
//{
...
//
//	// We suppose all eNBs have the same codebook and all the UE have the same codebook too.
//
//	complex2DVector_t enbCodebook, ueCodebook;
//	enbCodebook = LoadCodebookFile(txFilePath);
//	ueCodebook = LoadCodebookFile(rxFilePath);
//
//	for (NetDeviceContainer::Iterator it = enbDevices.Begin(); it != enbDevices.End(); it++)
//	{
//		Ptr<MmWaveEnbNetDevice> enbDev =
//								DynamicCast<MmWaveEnbNetDevice> (*it);
//		m_enbCodebooks.insert (std::pair<Ptr<NetDevice>,complex2DVector_t>(enbDev,ueCodebook));
//	}
//
//	for (NetDeviceContainer::Iterator it = ueDevices.Begin(); it != ueDevices.End(); it++)
//	{
//		Ptr<MmWaveUeNetDevice> UeDev =
//								DynamicCast<MmWaveUeNetDevice> (*it);
//		m_ueCodebooks.insert (std::pair<Ptr<NetDevice>,complex2DVector_t>(UeDev,ueCodebook));
//	}
//
//
//}
...
//
//void
//MmWaveBeamforming::BeamSweepTest (Ptr<const SpectrumValue> txPsd,
//									Ptr<const MobilityModel> a,
...
//		// .at (randomInstance);
//		Ptr<BeamformingParams> bfParams = it->second;
//		Ptr<BeamformingParams> tempBfParams(bfParams);
//
//		int numTxBeamId = 64, numRxBeamId = 8;
//		for (int txBeamId = 0; txBeamId < numTxBeamId; txBeamId++)
//		{
//			tempBfParams->m_enbW = enbBfs.at(txBeamId);
//			for (int rxBeamId = 0; rxBeamId < numRxBeamId; rxBeamId++)
//			{
//				tempBfParams->m_ueW = ueBfs.at(rxBeamId);
//				tempBfParams->m_beam = GetLongTermFading(tempBfParams);
//
//				Vector rxSpeed = b->GetVelocity();
//				Vector txSpeed = a->GetVelocity();
//				double relativeSpeed = (rxSpeed.x-txSpeed.x)
...
//				int nbands = bfGain.GetSpectrumModel ()->GetNumBands ();
//
//				std::cout << "txBeamId=" << txBeamId << ",rxBeamId=" << rxBeamId << ",bfGain=" << Sum (bfGain)/nbands <<",bfPsd=" << Sum(*bfPsd)/nbands << std::endl; //",rxPsd=" << Sum(*rxPsd)/nbands << std::endl;
//			}
//		}
//
//}
